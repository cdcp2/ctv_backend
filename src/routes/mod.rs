use crate::{
    db::DbPool,
    handlers::{advertisement, article, auth, category, live_stream, site_config, tag, upload},
    utils::jwt::{admin_middleware, admin_or_subadmin_middleware, auth_middleware}, // <--- Importamos ambos middlewares
};
use axum::{
    Router,
...
pub fn create_routes(pool: DbPool) -> Router {
    ...
    let public_routes = Router::new()
        ...
        .route(
            ...
            post(article::increment_views_handler),
        )
        .route(
            "/api/articles/:slug/tags",
            get(tag::list_article_tags_handler),
        )
        .route(
            "/api/site-config",
            get(site_config::get_site_config_handler),
        )
        .route("/api/tags", get(tag::list_tags_handler))
        .route(
            "/api/ads/positions",
            get(advertisement::list_ad_positions_handler),
        )
    ...
    let editor_routes = Router::new()
        ...
        .route(
            ...
            put(article::update_article_handler),
        ) // Editar sí dejamos a editores
        .route("/api/admin/tags", post(tag::create_tag_handler))
        .route(
            "/api/admin/articles/:id/tags",
            post(tag::set_article_tags_handler),
        )
        .route(
            "/api/upload",
            post(upload::upload_image_handler).layer(DefaultBodyLimit::disable()),
        )
    ...
    let admin_routes = Router::new()
        .route(
            "/api/admin/articles/:id",
            delete(article::delete_article_handler),
        )
        .route(
            "/api/admin/categories",
            post(category::create_category_handler),
        )
        .route(
    ...
    let site_config_routes = Router::new()
        .route(
            ...
            put(site_config::update_site_config_handler),
        )
    ...
    let ads_routes = Router::new()
        ...
        .route(
            "/api/admin/ads/:id",
            delete(advertisement::delete_ad_handler),
        )
        .route_layer(middleware::from_fn(admin_or_subadmin_middleware));
